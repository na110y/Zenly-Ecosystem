...
describe('requireVerifiedAdmin guard on a business admin route (real h3/HTTP)', () => {
  let server: Server
  let baseUrl: string
  ...
  it('returns 401 with no AdminSession cookie', async () => {
    const response = await fetch(`${baseUrl}/api/admin/business-route`, {
      headers: { Accept: 'application/json' },
    })
    expect(response.status).toBe(401)
  })

  it('returns 403 when the session exists but totpVerifiedAt is still NULL (login step 1 only)', async () => {
  ...
  it('returns 200 when totpVerifiedAt is set (full login complete)', async () => {
    findAdminSessionMock.mockResolvedValueOnce({
      adminAccountId: 'admin-1',
      tokenHash: hashToken('full-token'),
      revokedAt: null,
      expiresAt: new Date(Date.now() + 3600_000),
    ...
    const response = await fetch(`${baseUrl}/api/admin/business-route`, {
      headers: { Cookie: 'AdminSession=full-token', Accept: 'application/json' },
    })
    expect(response.status).toBe(200)
    expect(await response.json()).toEqual({ adminAccountId: 'admin-1' })
  })

  it('a UserSession cookie is never accepted (401, never reaches the 403 TOTP check)', async () => {
